...
public:
    static void swap(unsigned long long& x) { x = _byteswap_uint64(x); }
    static void swap(unsigned int& x)       { x = _byteswap_ulong(x); }
    static void swap(int& x)                { swap(reinterpret_cast<unsigned int&>(x)); }
    static void swap(unsigned short& x)     { x = _byteswap_ushort(x); }
};
...
namespace {
	...
	struct HavokEndianSwapImpl {
		...
		void convertElement(HAE::File* fp, int main_type) {
		...
		void convertStructure(HAE::File* fp, string type_name) {
		...
		void readData(HavokPackfileSectionHeader& header, HAE::File* fp) {
...
namespace HAE {
	...
	bool endianSwapHKXToFile(const std::string& inputPath, const std::string& outputPath) {
		File file(inputPath, "rb");
		if (!file.valid()) {
			return false;
		}

		vector<unsigned char> swapped = endianSwapHKX(&file);
		FILE* out = fopen(outputPath.c_str(), "wb");
		if (!out) {
			return false;
		}

		const bool ok = fwrite(swapped.data(), 1, swapped.size(), out) == swapped.size();
		fclose(out);
		return ok;
	}

	bool endianSwapHKXInPlace(const std::string& path) {
		return endianSwapHKXToFile(path, path);
	}
}
